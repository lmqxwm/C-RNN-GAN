...
def discriminative_score_metrics (ori_data, generated_data):
  ...
  def discriminator (x, t):
    ...
    with tf.compat.v1.variable_scope("discriminator", reuse = tf.compat.v1.AUTO_REUSE) as vs:
      d_cell = tf.compat.v1.nn.rnn_cell.GRUCell(num_units=hidden_dim, activation=tf.math.tanh, name = 'd_cell')
      ...
      y_hat = tf.math.sigmoid(y_hat_logit)
  ...
  d_loss_real = tf.compat.v1.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = y_logit_real, 
                                                                       labels = tf.ones_like(y_logit_real)))
  d_loss_fake = tf.compat.v1.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = y_logit_fake, 
